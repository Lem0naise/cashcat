...
export default function ManageBudgetModal({ isOpen, onClose }: ManageBudgetModalProps) {
    ...
    const fetchCategories = async () => {
        try {
            ...
            if (error) throw error;
            setCategories((data || []).map(cat => ({ ...cat, assigned: null })) as Category[]);
        } catch (error) {
    ...
    const updateCategory = async (id: string, categoryData: Partial<Category>) => {
        try {
            const promise = (async () => {
                // Filter out undefined/null values that shouldn't be sent to Supabase
                const updateData: any = {};
                Object.keys(categoryData).forEach(key => {
                    const value = categoryData[key as keyof Category];
                    if (value !== undefined && key !== 'assigned') {
