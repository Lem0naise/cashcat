...
type View =
    ...
    | 'set-category'     // bulk set category on selected
    | 'set-vendor'       // bulk reassign vendor on selected
    | 'vendor-category'  // set category for ALL transactions from a vendor
...
export default function BulkEditModal({ isOpen, onClose, initialSelection }: BulkEditModalProps) {
    ...
    // Vendor-category view: which vendor are we acting on?
    ...
    // Sorted vendor list for vendor-category view
                        ...
                        <div>
                            ...
                            {view === 'select' && userTransactions.length > 0 && (
                                <p className="text-xs text-white/40 mt-0.5">
                                    {selectedIds.size} of {userTransactions.length} selected
                                </p>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-1 min-h-0">
                            {filteredTransactions.length === 0 ? (
                                <div className="text-center text-white/40 py-12 text-sm">
                                    {userTransactions.length === 0 ? 'No transactions yet.' : 'No matches.'}
                                </div>
                            ) : (
                ...
                {view === 'actions' && (
                    <div className="flex flex-col flex-1 min-h-0 md:max-h-[80vh]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2 min-h-0">
                            ...
                            {/* ── Action: Categorise by vendor ── */}
                            <ActionButton
                                icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /></svg>}
                                label="Categorise All by Vendor"
                                description="Set a category for every transaction from a specific vendor across your entire history"
